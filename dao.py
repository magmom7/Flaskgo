...
class UserDAO:
    ...
    def userinsert(self, dto):  # 만약 속성값이 30개가 넘는다면 관리하기 힘드므로,,, DTO 객체 통으로 받음
        try:
            ...
            try:
                print("===1212==")
                cur.execute("insert into users values(:user_id, :user_name, :user_pw, :user_interset)", user_id=dto.getId(
                ), user_name=dto.getName(), user_pw=dto.getPw(), user_interset=dto.getInterest())
                conn.commit()
                print("===2==")
...
class BoardDAO:
    ...
    def textinsert(self, dto):  # 만약 속성값이 30개가 넘는다면 관리하기 힘드므로,,, DTO 객체 통으로 받음
        try:
            ...
            try:
                print("in")
                cur.execute("insert into board values(:text_idx, :title, :content, :input_date, :user_id)", text_idx=dto.getIdx(
                ), title=dto.getTitle(), content=dto.getContent(), input_date=dto.getInputDate(), user_id=dto.getId())
                conn.commit()
    ...
    def getuserID(self, name):
        try:
            ...
            try:
                cur.execute(
                    "select user_id from users where user_name = :getname", getname=name)
                # print("ininiin")
                row = cur.fetchone()
                # print("---row---", row)
